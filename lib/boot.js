...
exports.start = function (callback) {
    ...
    xFs
      ...
      .forEach (item => {
        if (
          ...
          serverConfig.modules.indexOf (item.def.name) !== -1
        ) {
          modules[item.def.name] = item;
          Object.assign (deps, ...Object.keys (item.def.dependencies));
        } else {
          modulesExtra[item.def.name] = item;
        }
      });
  });
  ...
  try {
    xBus.boot (commandHandlers, callback);
  } catch (ex) {
    callback (ex);
  }
};
