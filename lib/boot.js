      ...
      xFs
        ...
        .forEach(item => {
          if (
            ...
            extract(Object.keys(item.def.dependencies));
          }
        });
    });
  };

  let modulesList = serverConfig.modules;
  if (serverConfig.hasOwnProperty('modules') && hordeConfig.hordes) {
    modulesList = modulesList.concat(
      'xcraft-core-horde',
      hordeConfig.hordes.filter(
        appId =>
          !hordeConfig.topology ||
          (hordeConfig.topology && !hordeConfig.topology[appId])
      )
    );
  }
