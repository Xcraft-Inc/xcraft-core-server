...
exports.get = function() {
  const raceArray = Object.keys(motd);
  const race =
    raceArray[parseInt(Math.floor(Math.random() * raceArray.length))];

  const unitArray = Object.keys(motd[race]);
  const unit =
    unitArray[parseInt(Math.floor(Math.random() * unitArray.length))];

  const text =
    motd[race][unit][
      parseInt(Math.floor(Math.random() * motd[race][unit].length))
    ];
  return {
    race: race,
    unit: unit,
